...
run_child(void)
{
	...
	counter++;		/* Note that all "processes" share an address
				   space, so this change to 'counter' will be  					   visible to all processes. */
/***************************************************/ 
       // extra credit number 7 for check the answer
	/*int final_check;
	int index=1;
	if (sys_getpid()%2 == 0) 
	{
		while (index < NPROCS)
		{
			int final_check = sys_kill(index);
			//if (final_check == -1) {} // check process kill succesfully or not.
			index+=2;
		}
	}*/
/*****************************************************/
